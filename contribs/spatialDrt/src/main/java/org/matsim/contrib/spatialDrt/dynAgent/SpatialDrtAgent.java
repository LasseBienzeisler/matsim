...
public final class SpatialDrtAgent implements MobsimDriverPassengerAgent {
	...
	public SpatialDrtAgent(Id<Person> id, Id<Link> startLinkId, EventsManager events, DynAgentLogic agentLogic) {
		...
		state = Time.isUndefinedTime(dynActivity.getEndTime()) ? State.ABORT : State.ACTIVITY;
	}
	...
	private void computeNextAction(DynAction oldDynAction, double now) {
		...
		if (nextDynAction instanceof DynActivity) {
			...
			if (oldDynAction instanceof DynActivity && ((DynActivity) oldDynAction).getActivityType().equals(DrtActionCreator.DRT_QUEUE_NAME)){
				if (((DynActivity) nextDynAction).getActivityType().equals(DrtActionCreator.DRT_QUEUE_NAME)){
					return;
				}else{
	...
	public void endActivityAndComputeNextState(double now) {
		if (!dynActivity.getActivityType().equals(DrtActionCreator.DRT_QUEUE_NAME)) {
